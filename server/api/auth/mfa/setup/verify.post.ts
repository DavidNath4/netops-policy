...
import { toUserResponse, verifyMfaEnrollment } from '../../../../services/auth.service'
...
/**
 * POST /api/auth/mfa/setup/verify — confirm enrollment with the first code.
 *
 * Requires a valid MFA_ENROLLMENT challenge. On a valid code: enable MFA
 * (handled inside verifyMfaEnrollment), consume the challenge, create the
 * authenticated session cookie, and record last login. This is the point where
 * a full session first exists.
 */
export default defineEventHandler(async (event) => {
  ...
  const valid = await verifyMfaEnrollment(db, challenge.userId, parsed.data.code)
  if (!valid) {
    throw apiError(401, 'INVALID_VERIFICATION_CODE', 'Invalid verification code')
  }
